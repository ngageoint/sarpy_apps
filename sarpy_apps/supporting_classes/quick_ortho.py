...
class QuickOrtho:
    ...
    def create_ortho(self,
        ...
        image_points = numpy.zeros((display_image_nx * display_image_ny, 2))
        canvas_coords_1d = numpy.zeros(2 * display_image_nx * display_image_ny)

        tmp_x_vals = numpy.arange(0, display_image_ny)
        tmp_y_vals = numpy.zeros(display_image_ny)
        ...
        x = numpy.ravel(world_x_coordinates)
        y = numpy.ravel(world_y_coordinates)
        z = numpy.ravel(numpy.transpose(input_image_data))
        ...
        s = numpy.zeros((output_nx * output_ny, 3))
        ...
        mask = numpy.ones_like(gridded_image_pixels[0][:, 0])
        indices_1 = numpy.where(gridded_image_pixels[0][:, 0] < min(full_image_coords_y))
        indices_2 = numpy.where(gridded_image_pixels[0][:, 1] < min(full_image_coords_x))
        indices_3 = numpy.where(gridded_image_pixels[0][:, 0] > max(full_image_coords_y))
        indices_4 = numpy.where(gridded_image_pixels[0][:, 1] > max(full_image_coords_x))
        ...
        mask_2d = numpy.reshape(mask, (output_ny, output_nx))
    ...
    def _create_ground_grid(min_x,  # type: float
        ...
        ground_y_arr, ground_x_arr = numpy.mgrid[0:npix_y, 0:npix_x]
        ...
        x_gsd = numpy.abs(ground_x_arr[0, 1] - ground_x_arr[0, 0])
        y_gsd = numpy.abs(ground_y_arr[0, 0] - ground_y_arr[1, 0])
